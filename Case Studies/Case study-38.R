#Case study 34
#url = "http://www.reevesimportmotorcars.com/brands/bmw/new/inventory"

alldata.38 = function(url){
  require(RSelenium)
  require(XML)
  require(plyr)
  ...
  countElem = remDr$findElement('xpath','//span[@class="k-pager-info k-label"]')
  ...
  for(p in seq(1:(times+1)))
  {
    ...
    titleElems = remDr$findElements("xpath","//a[@class='car-listing-title']")
    ...
    vinStrs = unlist(lapply(titleElems, function(x){x$getElementAttribute("href")}))
    ...
    if(p<=times)
    {
      ...
      nextElem = remDr$findElement("xpath","//span[@class='k-icon k-i-arrow-e']") 
